...
public class StochasticGradientDescent {
	...
	protected void RunTraining() {
		...
		for (int idx = 0; idx < size; idx++) {
			...
			double sigmaSquare = this.relativeImportance*this.relativeImportance;

			this.learningRate = this.init_learningRate / (double)((double)1+this.init_learningRate*((double)(idx+1))/sigmaSquare);
			
			Vector x = this.trainingExamples.get(idx);
			
			double y = Double.parseDouble(x.Label);
			double wx = MathHelper.VectorDotProduct(this.weight, x);
			double para1 = ((double)-1*y/(1+Math.exp(y*wx)));
			Vector tmp1 = MathHelper.VectorScalarMultiplication(x, para1);
			Vector tmp2 = MathHelper.VectorScalarMultiplication(this.weight, (double)2/sigmaSquare);
			Vector gradient = MathHelper.VectorSum(tmp1, tmp2);
